...
gulp.task("deploy", gulp.series('jekyll', 'sass', 'copy' , function() {
	return cp.spawn('git status && git commit -am "Update docs folder for GHPages + animation fixes, single record animation removed" && git pull && git push', { stdio: "inherit", shell: true });
}));